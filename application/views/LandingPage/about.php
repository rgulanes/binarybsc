<!DOCTYPE html>
<!--[if lt IE 7 ]><html class="ie ie6" lang="en"> <![endif]-->
<!--[if IE 7 ]><html class="ie ie7" lang="en"> <![endif]-->
<!--[if IE 8 ]><html class="ie ie8" lang="en"> <![endif]-->
<!--[if (gte IE 9)|!(IE)]><!-->
<html lang="en">
<!--<![endif]-->
<!-- HEAD SECTION -->
<?php  $this->load->view('Globals/LandingPage/header');?>
<!--END HEAD SECTION -->
<body>
    <!-- NAV SECTION -->
        <?php  $this->load->view('Globals/LandingPage/navbar');?>
    <!--END NAV SECTION -->
    <!-- ABOUT SECTION -->
    <div class="section">
        <div class="container">

            <div class="row main-low-margin">
                <div class="col-md-10 col-md-offset-1 col-sm-10 col-sm-offset-1">
                    <h1>About OSA Brokenshire</h1>
                    <h5>Student Affairs Office caters the following: Student Organizations, Student Conduct and Discipline,  Scholarship and Discounts, Non-Academic Awards, Student Activities, Campus Publicity, Certificate of Good Moral, Waiver/ Parental Consent, School Uniform/ID/ Logo, Mails , Lost & Found Items, Confiscated/Found IDs, Lockers.
                    </h5>
                </div>
            </div>


        </div>
    </div>

    <div class="container">
    
    <h2>Office of the Student Affairs | Head & Staff</h2>
        <div class="row main-low-margin ">

            
                <div class="col-md-12 col-sm-12 text-center">
                    <img class="img-circle" src="assets/landingpage/img/osa.jpg" style="width: 200px" >
                    <h3>Office of Student Affairs | Head</h3>
                    <h4>Ms. Joyce Jasa, MAEd</h4>
                </div>

                <div class="col-md-6 col-sm-6 text-center">
                    <img class="img-circle" src="assets/landingpage/img/img1.jpg" style="width: 200px" >
                    <h3>Office of Student Affairs | Athletics In-Charge </h3>
                    <h4>Mr. Rodne T. Baslot</h4>
                </div>

                <div class="col-md-6 col-sm-6 text-center">
                    <img class="img-circle" src="assets/landingpage/img/img1.jpg" style="width: 200px" >
                    <h3>Office of Student Affairs | OSA Staff-Scholarship </h3>
                    <h4>Mr. Richard E. Andante</h4>
                </div>

        </div>
        <h2>Student Assistants</h2>
        <div class="row main-low-margin text-center">
            <div class="col-md-6 col-sm-6 text-center">
                <img class="img-circle" src="assets/landingpage/img/img1.jpg">
                <h3>Ms. Jachin D. Mendez</h3>
                <h4>Student Assistant</h4>
                
            </div>
            <div class="col-md-6 col-sm-6 text-center">
                <img class="img-circle" src="assets/landingpage/img/img1.jpg">
                <h3>Mr. Jestone A. Divinagracia</h3>
                <h4>Student Assistant</h4>
               
            </div>
           
        </div>
     
        
    </div>
    <div class="space-bottom"></div>
    <!--END ABOUT SECTION -->
    
    <?php  $this->load->view('Globals/LandingPage/footer');?>

    <?php  $this->load->view('Globals/LandingPage/scripts');?>
 
</body>
</html>
